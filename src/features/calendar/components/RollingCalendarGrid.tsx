          ...
          return (
            ...
            >
              ...
              <span className="rcg-day-number">
                {isNewMonth || day.isSame(days[0], 'day') ? day.format('D MMM') : day.format('D')}
              </span>
              ...
              {hasWorkout && workout && (
                <div className="rcg-workout-indicator">
                  <span
                    ...
                    style={{
                      background: INTENSITY_STYLES[workout.intensity]?.gradient,
                    }}
                    title={`${INTENSITY_STYLES[workout.intensity]?.label} — ${workout.workoutCount} workout${workout.workoutCount > 1 ? 's' : ''}`}
                  />
                  <span className="rcg-workout-count">{workout.workoutCount}</span>
                </div>
              )}
            </button>
          )
