...
export const ServerRoute = createServerFileRoute('/api/workout-sets').methods({
  ...
  POST: privateMethod(async ({ supabase, request }) => {
  ...
  PUT: privateMethod(async ({ supabase, request, url }) => {
    const setId = url.searchParams.get('id')
  ...
  DELETE: privateMethod(async ({ supabase, url }) => {
    ...
    if (workoutId && exerciseId) {
      ...
      // `deleted` is nested into the envelope's `data` field per Path X
      // uniform envelope. The only client (workout-session/client.ts)
      // discards the body via readApiSuccess.
      return { deleted }
    }
